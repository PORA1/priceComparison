...
if(isset($_POST['uploadcommodity']))
{
	$commodityname	= mysqli_real_escape_string($connection, $_POST['commodityname']);	
	$commoditybrand	= mysqli_real_escape_string($connection, $_POST['commoditybrand']);
...
//error handlers for the upload form 
	...
	if(empty($commoditybrand)){
		array_push($errors, "Commodity Brand is required");
	}
	...
	if(count($errors) == 0){
		...
		$insert_commodity = mysqli_query($connection, "INSERT INTO commodity_table (commodityname,commoditybrand,commodityprice,commodityimage,idvendor) VALUES ('$commodityname','$commoditybrand','$commodityprice','$commodityimageNameNew','$vendorid')");
		echo mysqli_error($connection);
